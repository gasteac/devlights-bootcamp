...
const filteredList = watchList.filter(movie => parseInt(movie.imdbRating) >= 8).map(movie => ({title: movie['Title'],rating: movie['imdbRating']}));
...
function getRating(watchList) {
  ...
  .reduce((sum, rating) => sum + rating) / watchList
...
const squareList = arr => {
  // Only change code below this line
  arr = arr
  .filter(number => number > 0 && number == parseInt(number))
